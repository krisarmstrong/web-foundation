...
function countUsage(repoRoot, tokens){
  ...
  for(const f of files){
    ...
    for(const t of tokens){
      const suffix = t;
      const needle = `var(--theme-${suffix})`;
      if(data.includes(needle)){
        const matches = data.match(new RegExp(escapeRegExp(needle), 'g')) || [];
...
(function run(){
  const repoRoot = path.resolve(__dirname, '..'); // repo root (web-foundation)
  const tokensPath = path.join(repoRoot, 'src', 'themes', 'shared-tokens.css');
  const tokens = tokensFromCss(tokensPath);
  const usage = countUsage(repoRoot, tokens);
  const unused = tokens.filter(t => usage[t] === 0);
  const out = { repo: 'web-foundation', tokens: tokens, usage: usage, unused: unused };
